...
- **Fixes production errors from Sentry** — with `SENTRY_AUTH_TOKEN` + `SENTRY_ORG` configured, the worker watches Sentry for recurring error groups, turns valid ones into bugfix tasks, and ships the fix through the same pipeline as a reviewed pull request.
