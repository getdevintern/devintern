...
- **Fixes production errors from Sentry** — add a Sentry project during `worker init` (or configure `[[error_monitors]]` later) and the worker watches recurring error groups, turns valid ones into bugfix tasks, and ships the fix through the same pipeline as a reviewed pull request.
