...
`@getdevintern/pm` and other callers that use `runAgentNode` / `runAgentBun` run Muse via the dedicated `runAgentMuse` path (`muse exec --json`), which parses JSONL stdout incrementally, maps Muse-specific exit states, and auto-uses `--prompt-file` for large prompts (tune with `MUSE_PROMPT_FILE_THRESHOLD_BYTES`). **`devintern` (`@getdevintern/code`) still uses the generic agent spawn path today**, so Muse runs emit raw JSONL to the terminal/JIRA output, do not auto-deliver large prompts via `--prompt-file`, treat exit code `0` as success without Muse JSONL step-limit detection, and do not surface Muse `exitState` from JSONL events. Full Muse integration in `devintern` is planned as a follow-up.
