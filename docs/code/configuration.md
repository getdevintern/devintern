...
**Muse Code note:** Harness id is `muse`; the CLI binary is `muse`. Install and authenticate per [Muse Code authentication](https://dev.meta.ai/docs/muse-code/auth), then set `AGENT_HARNESS=muse`. DevIntern runs headless via `muse exec --json`, parses JSONL stdout incrementally, and maps Muse exit codes separately from task/test success (exit `0` means the turn finished, not that your tests passed). Unattended runs default to `--disable-approval` (sandbox stays on). `--yolo` disables both approval and sandbox and must be enabled explicitly in harness options — never use it on untrusted PR/fork checkouts. Map DevIntern `--max-turns` to Muse `--max-model-steps`. Muse has no native plan/readonly headless mode. On Linux CI, sandboxed runs need working Bubblewrap and a non-musl Muse build; see [Muse permissions](https://dev.meta.ai/docs/muse-code/permissions#sandbox). Optional path override: `MUSE_CLI_PATH`. Large prompts may be written to a private temp file (`--prompt-file`) automatically; tune with `MUSE_PROMPT_FILE_THRESHOLD_BYTES`.
