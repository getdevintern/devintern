...
- **Run list**: every run with its task key or automation id, origin (tracker task, PR mention, scheduled, or estimate), agent harness, status, result, start time, and duration. The task key links straight to the tracker ticket when the tracker's URL can be derived from your configuration; filter by status or origin with the compact dropdown filters above the table (`origin=scheduled` isolates automation runs; `origin=estimate` isolates story-point sweeps). The harness and git branch are recorded when the run starts, so runs from before they were recorded show `–`. The branch is shown on the run detail page rather than in the list, where long branch names crowd out the columns that matter for scanning.
