...
- All worker state (queue, cursors, agent PR registry, run records, routing skips) lives in one database at `~/.devintern/state/queue.db`. Durable config that a task run would otherwise drop beside its checkout — the `.pid.lock`, the auth session, and the license cache — is pinned to `~/.devintern/.devintern-code/` via `DEVINTERN_CONFIG_DIR`, so a task can never write `.devintern-code/` runtime files into the repository it is editing. As a backstop, each worker-managed worktree lists `.devintern-code/` in its clone's `.git/info/exclude`, so even an unexpected write stays out of the commit (checkouts under `/tmp` made from your own repository are never modified). A repo that intentionally commits `.devintern-code/settings.json` keeps it tracked and unchanged.
