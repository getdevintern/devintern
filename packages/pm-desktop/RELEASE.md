...
### Error reporting (build-time)

| Repository setting | Kind           | Maps to env                                                                                                                                          |
| ------------------ | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SENTRY_DSN`       | Actions secret | Baked into the main bundle via `electron.vite.config.ts` at package time. Missing → error tracking permanently no-op in that build. Users can opt out via Settings (shares the analytics toggle). |
