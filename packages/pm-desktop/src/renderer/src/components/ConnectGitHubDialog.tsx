...
const GITHUB_APP_INSTALL_URL = "https://github.com/apps/devintern-pm/installations/new";
...
function OAuthCodePrompt({ prompt }: { prompt: OAuthPrompt }) {
  return (
    <div className="space-y-3" data-testid="connect-github-oauth-code">
      ...
      <div className="flex items-center gap-2">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          onClick={() => void navigator.clipboard?.writeText(prompt.userCode)}
      ...
      <p className="text-xs text-muted-foreground">
        A browser tab opened to {prompt.verificationUri}. If it closed, open it again below.
      </p>
      <Button
        type="button"
        variant="link"
        className="h-auto px-0 text-xs"
        onClick={() => void window.pm.openExternal(prompt.verificationUri)}
      >
...
function OAuthSignInButton({
  ...
  return (
    <>
      <p className="text-xs text-muted-foreground">
        Authorize the DevIntern PM app to access your repositories. Opens your browser — no token to
        paste.
      </p>
      <Button
        type="button"
...
function PatSection({
  ...
  return (
    <div className="space-y-2" data-testid="connect-github-pat">
      ...
      <p className="text-xs text-muted-foreground">
        Required for private repos. Fine-grained: Contents Read (and Metadata). Classic: repo scope.
        Stored only on this machine (encrypted when the OS supports it).
      </p>
      ...
      {oauthAvailable && authRequired ? (
        <div className="space-y-2" data-testid="connect-github-oauth">
          ...
          {oauthRunning && oauthPrompt ? (
            <OAuthCodePrompt prompt={oauthPrompt} />
          ) : (
            <OAuthSignInButton oauthRunning={oauthRunning} onSignIn={onSignIn} />
          )}
...
function ConnectedBanner({ authMethod, onSwap }: { authMethod?: string; onSwap: () => void }) {
  return (
    <p className="text-xs text-muted-foreground" data-testid="connect-github-auth-ok">
      ...
      <Button type="button" variant="link" className="h-auto px-1 text-xs" onClick={onSwap}>
        Change
      </Button>
    </p>
  );
  ...
  return (
    ...
    >
      <p className="text-xs text-foreground">
        No repositories found. Install the DevIntern PM app on your GitHub account or organization
      ...
      <Button
        ...
        variant="outline"
        size="sm"
        onClick={() => void window.pm.openExternal(GITHUB_APP_INSTALL_URL)}
...
function RepoList({
  ...
  return (
    <div className="max-h-36 space-y-1 overflow-y-auto" data-testid="connect-github-repo-list">
      {repos.length > 0 ? (
        repos.map((repo) => (
          <button
            key={repo.fullName}
...
function PublicOnlyNotice() {
  return (
    ...
    >
      <p className="text-xs text-foreground">
        Only public repositories are showing. To access private repos, add them to the DevIntern PM
        app on GitHub.
      </p>
      <Button
        type="button"
...
interface RepoPickerProps {
  repos: GitHubRepoListItem[];
  filteredRepos: GitHubRepoListItem[];
...
function RepoPicker({
  ...
  const isOAuth = authConnected && authMethod === "oauth" && !loadingRepos;
  ...
  return (
    <div className="space-y-2">
      ...
      />
      {authConnected && hasRepos ? <RepoList repos={filteredRepos} onPick={onPickRepo} /> : null}
      {loadingRepos ? (
        <p className="text-xs text-muted-foreground">Loading your repositories…</p>
      ) : null}
      {isOAuth && hasRepos && !hasPrivateRepos ? <PublicOnlyNotice /> : null}
      {isOAuth && hasPrivateRepos ? <ManageAccessButton /> : null}
    </div>
  );
}
...
function DialogActions({
  ...
  return (
    <DialogFooter>
      <Button type="button" variant="outline" disabled={connecting} onClick={onCancel}>
        Cancel
      </Button>
      <Button
        type="button"
...
export function ConnectGitHubDialog({ open, onOpenChange, onConnected }: ConnectGitHubDialogProps) {
  ...
  const [oauthPrompt, setOauthPrompt] = useState<OAuthPrompt | null>(null);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg" data-testid="connect-github-dialog">
        ...
        <div className="space-y-4 py-1">
          {showAuthBlock ? (
            <AuthBlock
              ...
              savingToken={savingToken}
              connecting={connecting}
              onSignIn={() => void onSignInWithGitHub()}
              onCancelOAuth={() => void window.pm.cancelGitHubOAuth()}
              onShowPat={setShowPat}
              onTokenChange={setToken}
              onSaveToken={() => void onSaveToken()}
            />
          ) : null}
          ...
          {needsInstall && authConnected ? <NeedsInstallNotice /> : null}
          ...
          <RepoPicker
            ...
            onPickRepo={setRepoInput}
          />

          <BranchField branch={branch} connecting={connecting} onChange={setBranch} />
        ...
        <DialogActions
          connecting={connecting}
          canConnect={Boolean(repoInput.trim())}
