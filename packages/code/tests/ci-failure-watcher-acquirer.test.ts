...
describe("CiFailureWatcherAcquirer", () => {
  ...
  function makeGithub(gh: FakeGitHubState): CiFailureWatcherGitHub {
    return {
      ...
      async fetchCommitStatus(_repo, _sha, etag): Promise<CiConditionalResult<WatchedStatusState>> {
        ...
        if (!gh.status || gh.statusEtagHit) {
          return { data: gh.status ?? null, etag, notModified: Boolean(gh.statusEtagHit) };
        }
        return { data: gh.status, etag: 'W/"status-1"', notModified: false };
      },
  ...
  function makeAcquirer(gh: FakeGitHubState, overrides: { maxAttempts?: number } = {}) {
    const fixed: string[] = [];
  ...
  test("CI success resets the retry counter", async () => {
    ...
    // Escalation was posted and no further attempt happens at the same head.
    await acquirer.tick();
