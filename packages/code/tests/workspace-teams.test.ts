...
describe("multi-team polling", () => {
  ...
  test("single-defaults behavior is unchanged (regression)", async () => {
    ...
    expect(executed.map((r) => r.taskKey)).toEqual(["T-1"]);
    expect(state.workerState.getCursor("markdown")?.cursorValue).toBe("1");
    // Skips stay team-less outside [[teams]] mode.
    const skip = state.skips.latestFor("T-2");
    expect(skip?.reason).toBe("unrouted");
    expect(skip?.team).toBeUndefined();
  });
});
...
describe("relay task evaluation across teams", () => {
  ...
  test("the first source whose query matches executes the task", async () => {
    const ran: string[] = [];
    const dispatch = createFleetRelayTaskDispatcher({
      sources: [
        { label: "platform", evaluate: evaluatorFor(findTeam(CONFIG, "platform"), [], ran) },
        {
          label: "growth",
  ...
  test("a task matching several sources runs exactly once", async () => {
    const ran: string[] = [];
    const shared = (): FleetTask[] => [{ key: "77", updated: "u1", labels: ["docs"] }];
    const dispatch = createFleetRelayTaskDispatcher({
      sources: [
        { label: "platform", evaluate: evaluatorFor(findTeam(CONFIG, "platform"), shared(), ran) },
        { label: "growth", evaluate: evaluatorFor(findTeam(CONFIG, "growth"), shared(), ran) },
      ],
    });

    await dispatch("77");
    expect(ran).toEqual(["platform:77"]);
  });
  ...
  test("tasks matching no source are ignored", async () => {
    ...
    const dispatch = createFleetRelayTaskDispatcher({
      ...
      sources: [
        { label: "platform", evaluate: evaluatorFor(findTeam(CONFIG, "platform"), [], ran) },
        { label: "growth", evaluate: evaluatorFor(findTeam(CONFIG, "growth"), [], ran) },
      ],
    });

    await dispatch("MISSING-1");
    expect(ran).toEqual([]);
  });
});
