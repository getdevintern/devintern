...
- **Optional Sentry action comments**: `comment_on_action = true` leaves a best-effort comment after a terminal successful or failed remediation run without resolving the issue or changing its workflow fields. Deferred runs stay silent, comment failures never affect execution or deduplication, and the option remains off by default because Sentry's private comment endpoint requires a user-authenticated token with Issue & Event write access
