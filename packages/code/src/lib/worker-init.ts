 ...
 * can install and launch a native user service (systemd on Linux, launchd on
 * macOS) — or just write its definition and print the manual steps. Polling
 * is always on; direct webhooks run as a separate advanced service.
 *
...
export async function runWorkerInit(deps: WorkerInitDeps = {}): Promise<WorkerInitResult> {
  ...
  try {
    ...
    // 7. Offer to install and launch the native user service. The foreground
