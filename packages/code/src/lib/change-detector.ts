 ...
 */
export function createGitLabChangeDetector(
  searchTasks: SearchTasksFn,
  options: ChangeDetectorOptions = {},
): ChangeDetector {
  return createQueryChangeDetector(options.source ?? "gitlab", searchTasks, (since) => {
    const iso = new Date(since).toISOString().replace(/\.\d{3}Z$/, "Z");
    return `updated:>=${iso}`;
  });
}
...
export function createChangeDetector(
  ...
  switch (trackerType.toLowerCase()) {
    ...
    case "gitlab":
      return searchTasks ? createGitLabChangeDetector(searchTasks, options) : null;
    case "azure-devops":
