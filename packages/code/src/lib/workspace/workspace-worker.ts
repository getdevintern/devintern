...
export async function runWorkspaceWorker(options: RunWorkspaceWorkerOptions): Promise<void> {
  ...
  if (config.workspace.dashboard) {
    try {
      ...
      startDashboardServer({ port: config.workspace.dashboardPort, workingDir: workspaceDir });
    } catch (error) {
      console.warn(
  ...
  if (relayState || process.env.WORKER_RELAY_URL) {
    ...
    if (!relayToken) {
      ...
      const execute = createFleetTaskExecutor({
        ...
        skips: state.skips,
        repoManager,
        // Relay task events join the same per-repo lanes and global limit as
        // polling, reviews, and mentions.
        scheduler,
      });
