...
import { HarnessFallbackCoordinator, preExhaustedFor } from "./lib/harness-fallback";
import { exitClassificationError } from "./lib/harness-launch";
import { runCapturedAgentProcess } from "./lib/harness-process";
 ...
 * configured primary, except candidates this process already disqualified as
 * executable-missing in an earlier attempt — re-attempting a known-dead
 * binary would pay the full ENOENT retry budget per stage in batch mode.
 */
function createHarnessFallbackCoordinator(): HarnessFallbackCoordinator {
  ...
  return new HarnessFallbackCoordinator(candidates, {
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      ...
      const {
        ...
        timeoutMinutes,
        timeoutMessage: `${harness.displayName} clarity check timed out after ${timeoutMinutes} minutes`,
      });
      ...
      if (code === 0) {
        ...
        reject(
          exitClassificationError(
            `Agent clarity check exited with code ${code}`,
            stdoutOutput,
            stderrOutput,
            code,
          ),
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      ...
      const {
        ...
        exitSubject: "Agent estimation",
        timeoutMinutes,
        timeoutMessage: `Agent estimation timed out after ${timeoutMinutes} minutes`,
        // A usage/rate limit is account-global — abort instead of treating
        // this as a normal failure, before any exit classification.
        inspectClose: ({ stdout, stderr }) => {
          const usage = detectUsageLimit(stdout, stderr);
          if (!usage.limited) {
            return null;
          }
          ...
          return new UsageLimitError(usage.resetsAt);
        },
      });
      ...
      if (code !== 0) {
        ...
        reject(
          exitClassificationError(
            `Agent estimation exited with code ${code}`,
            stdoutOutput,
            stderrOutput,
            code,
          ),
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      ...
      const {
        ...
        exitSubject: "Agent",
        timeoutMinutes,
        timeoutMessage: `${harness.displayName} timed out after ${timeoutMinutes} minutes`,
        ...
        onNonZeroExit: (nonZeroCode) => {
          console.log(`❌ Agent exited with non-zero code ${nonZeroCode}`);
          console.log("   No JIRA comment will be posted due to execution failure");
        },
        inspectClose: async ({ stdout, stderr }) => {
          ...
          const matchedLine = findMaxTurnsReachedLine(stdout, stderr);
          if (matchedLine) {
            console.log(`   Matched output: ${matchedLine}`);
          }

          // Save incomplete implementation for analysis
          if (taskKey && stdout.trim()) {
            try {
              const baseOutputDir = resolveOutputDir();
              const taskDir = join(baseOutputDir, taskKey.toLowerCase());
