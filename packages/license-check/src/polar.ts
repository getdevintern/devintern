/**
 * Pure Polar licensing primitives.
 *
 * Zero-dependency module (fetch only, no Bun/node APIs) so it can run in any
 * runtime — the CLI, the website, and the Mode 2 control-plane worker
 * (Cloudflare) all share the same benefit table and key validation.
 */
